...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> resultRecord = this.root.put(key, rid);
        if(resultRecord.isPresent()) {
            this.updateRoot(resultRecord);
        }
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        while(data.hasNext()) {
            Optional<Pair<DataBox, Long>> popUpData = this.root.bulkLoad(data, fillFactor);
            if(popUpData.isPresent()) {
                this.updateRoot(popUpData);
            }
        }
    }

    private void updateRoot(Optional<Pair<DataBox, Long>> popUpData) {
        List<DataBox> keys = new ArrayList<>();
        List<Long> children = new ArrayList<>();

        keys.add(popUpData.get().getFirst());
        children.add(this.root.getPage().getPageNum());
        children.add(popUpData.get().getSecond());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        LeafNode curNode;
        Iterator<RecordId> curLeafIter;

        public BPlusTreeIterator(BPlusNode node, Optional<DataBox> key) {
            if(key.isPresent()) {
                this.curNode = root.get(key.get());
                this.curLeafIter = this.curNode.scanGreaterEqual(key.get());
            } else {
                curNode = root.getLeftmostLeaf();
                this.curLeafIter = this.curNode.scanAll();
            }
        }

        public BPlusTreeIterator(BPlusNode node) {
        ...
        public boolean hasNext() {
            if(this.curLeafIter.hasNext()) {
                return true;
            }
            Optional<LeafNode> rightSibling = this.curNode.getRightSibling();
            while(rightSibling.isPresent()) {
                curNode = rightSibling.get();
                curLeafIter = curNode.scanAll();
                if(curLeafIter.hasNext()) {
                    return true;
                } else {
        ...
        public RecordId next() {
            if(this.curLeafIter.hasNext()) {
                return this.curLeafIter.next();
            }
            throw new NoSuchElementException("No next element in BPlusTree iterator.");
        }
    }
}
