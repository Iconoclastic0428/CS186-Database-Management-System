...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Long recordId = pair.get().getSecond();

        int position = Collections.binarySearch(keys, key);
        position = -position - 1;

        this.keys.add(position, newKey);
        this.children.add(position + 1, recordId);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        int maximum = this.metadata.getOrder() * 2;
        while(this.keys.size() < maximum && data.hasNext()) {
            BPlusNode rightmost = this.getChild(this.keys.size());
            Optional<Pair<DataBox, Long>> dataPair = rightmost.bulkLoad(data, fillFactor);
            if(dataPair.isPresent()) {
                this.keys.add(dataPair.get().getFirst());
                this.children.add(dataPair.get().getSecond());
            }
        }
    ...
    private Optional<Pair<DataBox, Long>> split() {
        ...
        while(keys.size() > this.metadata.getOrder()) {
            newKeys.add(this.keys.remove(this.metadata.getOrder()));
            newChildren.add(this.children.remove(this.metadata.getOrder() + 1));
        }
        DataBox returnKey = newKeys.remove(0);
        InnerNode newNode = new InnerNode(this.metadata, this.bufferManager, newKeys, newChildren, this.treeContext);
        this.sync();
        newNode.sync();
        return Optional.of(new Pair<>(returnKey, newNode.getPage().getPageNum()));
    }
