...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        if(this.keys.contains(key)) {
            throw new BPlusTreeException("Duplicate key detected, record id = " + rid.toString() + '.');
        }
        ...
        for(int i = 0; i <= metadata.getOrder(); ++i) {
            ...
            newRids.add(this.rids.remove(this.metadata.getOrder()));
        }
        LeafNode newNode = new LeafNode(this.metadata, this.bufferManager, newKeys, newRids, this.rightSibling, this.treeContext);
        this.rightSibling = Optional.of(newNode.getPage().getPageNum());
        this.sync();
        newNode.sync();
        return Optional.of(new Pair<>(newNode.keys.get(0), this.rightSibling.get()));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        int maximum = Math.round(this.metadata.getOrder() * 2 * fillFactor);
        while(this.keys.size() <= maximum && data.hasNext()) {
            Pair<DataBox, RecordId> dataPair = data.next();
            this.keys.add(dataPair.getFirst());
            this.rids.add(dataPair.getSecond());
        }
        ...
        List<DataBox> newKeys = new ArrayList<>();
        List<RecordId> newRids = new ArrayList<>();
        newKeys.add(this.keys.remove(maximum));
        newRids.add(this.rids.remove(maximum));
    ...
    public void remove(DataBox key) {
        ...
        this.rids.remove(this.getKey(key).get());
